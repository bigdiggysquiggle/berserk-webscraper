...
base_url = "https://oyasumi-punpun-manga.com"
...
def fix_imgs():
	...
	for file in files:
		with Image.open(file) as fixing:
			print(file)
			fixing.save("fixed " + file.split('.')[0] + ".jpeg")
		os.remove(file)
		os.rename("fixed " + file.split('.')[0] + ".jpeg", file)

def dl_chap(chapter):
	name = chapter.text
	chapter=BeautifulSoup(requests.get(base_url + chapter.a['href']).text, 'html.parser')
...
if __name__ == '__main__':
	berserk_dl.setup_dirs("Goodnight Punpun")
